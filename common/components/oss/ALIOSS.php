...
use common\components\oss\util\MimeTypes;
use common\components\oss\lib\requestcore\RequestCore;
use common\components\oss\lib\requestcore\ResponseCore;
use common\components\oss\OSS_Exception;
/**
...
//加载MimeTypes
//require_once OSS_API_PATH.DIRECTORY_SEPARATOR.'util'.DIRECTORY_SEPARATOR.'MimeTypes.php';
...
if(file_exists(OSS_API_PATH.DIRECTORY_SEPARATOR.'lang'.DIRECTORY_SEPARATOR.ALI_LANG.'.inc.php')){
	...
	throw new common\components\oss\OSS_Exception(OSS_LANG_FILE_NOT_EXIST);
}
...
if(function_exists('get_loaded_extensions')){
	...
	if($extensions){
		if(!in_array('curl', $extensions)){
			throw new \common\components\oss\OSS_Exception(OSS_CURL_EXTENSION_MUST_BE_LOAD);
		}
	}else{
		throw new \common\components\oss\OSS_Exception(OSS_NO_ANY_EXTENSIONS_LOADED);
	}
}else{
	throw new \common\components\oss\OSS_Exception('Function get_loaded_extensions has been disabled,Pls check php config.');
}
...
class ALIOSS{
    ...
    //mibodemo.oss-cn-shanghai.aliyuncs.com
	/**
	 * OSS服务地址
	 */
     ...
     * @return string
     */
    public function get_sign_url_new($bucket, $object, $timeout = 60,$method=self::OSS_HTTP_GET, $options = NULL)
    {
        //options
        $this->validate_options($options);
