...
abstract class TimRestInterface
{
	 ...
	 */
//	static abstract function init($sdkappid, $identifier);
	 ...
	 */
	static abstract public function generate_user_sig($identifier, $expiry_after, $protected_key_path, $tool_path);
	 ...
	 * @return bool 返回成功与否
	 */
	static abstract public function set_user_sig($usr_sig);
	 ...
	 */
	static abstract function openim_send_msg($account_id, $receiver, $text_content);
	 ...
	 * @return array 通过解析REST接口json返回包得到的关联数组, 其中包含成功与否、及错误提示(如果有错误)等字段
	 */
	static abstract function openim_send_msg_pic($account_id, $receiver, $pic_path);
	 ...
	 * @return array 通过解析REST接口json返回包得到的关联数组, 其中包含成功与否、及错误提示(如果有错误)等字段
	 */
	static abstract function openim_send_msg2($account_id, $receiver, $msg_content);
	 ...
	 * @return array 通过解析REST接口json返回包得到的关联数组, 其中包含成功与否、及错误提示(如果有错误)等字段
	 */
	static abstract function openim_batch_sendmsg($account_list, $text_content);
	 ...
	 * @return array 通过解析REST接口json返回包得到的关联数组, 其中包含成功与否、及错误提示(如果有错误)等字段
	 */
	static abstract function openim_batch_sendmsg_pic($account_list, $pic_path);
	 ...
	 * @return array 通过解析REST接口json返回包得到的关联数组, 其中包含成功与否、及错误提示(如果有错误)等字段
	 */
	static abstract function openim_batch_sendmsg2($account_list, $msg_content);
     ...
     */
    static abstract function account_import($identifier, $nick, $face_url);
     ...
     * @return array 通过解析REST接口json返回包得到的关联数组，包含成功与否、错误提示等字段
     */
    static abstract public function register_account($identifier, $identifierType, $password);
	 ...
	 */
	static abstract function profile_portrait_get($account_id);
	 ...
	 * @return array 通过解析REST接口json返回包得到的关联数组，包含成功与否、拉取到的用户信息(如果成功)，及错误提示等字段
	 */
	static abstract function profile_portrait_get2($account_list, $tag_list);
	 ...
	 * @return array 通过解析REST接口json返回包得到的关联数组，包含成功与否、错误提示等字段
	 */
	static abstract function profile_portrait_set($account_id, $new_name);
	...
	static abstract function profile_portrait_set2($account_id, $profile_list);
	 ...
	 * @return array 通过解析REST接口json返回包得到的关联数组，包含成功与否、错误提示等字段
	 */
	static abstract function sns_friend_import($accout_id, $receiver);
	 ...
	 * @return array 通过解析REST接口json返回包得到的关联数组，包含成功与否、错误提示等字段
	 */
	static abstract function sns_friend_delete($account_id, $frd_id);
	 ...
	 * @return array 通过解析REST接口json返回包得到的关联数组，包含成功与否、错误提示等字段
	 */
	static abstract function sns_friend_delete_all($account_id);
	 ...
	 */
	static abstract function sns_friend_check($account_id, $to_account);
	 ...
	 * @return array 通过解析REST接口json返回包得到的关联数组，包含成功与否、错误提示、验证结果等字段
	 */
	static abstract function sns_friend_check2($account_id, $to_account_list, $check_type);
	 ...
	 */
	static abstract function sns_friend_get_all($account_id);
	 ...
	 * @return array 通过解析REST接口json返回包得到的关联数组，包含拉取到的好友信息、成功与否及错误提示等字段
	 */
	static abstract function sns_friend_get_all2($account_id, $tag_list);
	 ...
	 * @return array 通过解析REST接口json返回包得到的关联数组，包含拉取到的好友信息、成功与否及错误提示等字段
	 */
	static abstract function sns_friend_get_list($account_id, $frd_id);
	 ...
	 * @return array 通过解析REST接口json返回包得到的关联数组，包含拉取到的好友信息、成功与否及错误提示等字段
	 */
	static abstract function sns_friend_get_list2($account_id, $frd_list, $tag_list);
	 ...
	 */
	static abstract function group_get_appid_group_list();
	 ...
	 * @return array 通过解析REST接口json返回包得到的关联数组，包含拉取到的群组信息、成功与否及错误提示等字段
	 */
	static abstract function group_get_appid_group_list2($limit, $offset, $group_type);
	 ...
	 */
	static abstract function group_create_group($group_type, $group_name, $owner_id);
	 ...
	 * @return array 通过解析REST接口json返回包得到的关联数组，包含新建的群号、成功与否、错误提示等字段
     */
    static abstract function group_create_group2($group_type, $group_name, $owner_id, $info_set, $mem_list);
     ...
     * @return array 通过解析REST接口json返回包得到的关联数组，包含成功与否、错误提示等字段
     */
    static abstract function group_change_group_owner($group_id, $new_owner);
	 ...
	 */
	static abstract function group_get_group_info($group_id);
	 ...
	 * @return array 通过解析REST接口json返回包得到的关联数组，包含拉取到的群组信息(如果成功)、成功与否、错误提示等字段
	 */
	static abstract function group_get_group_info2($group_list, $base_info_filter, $member_info_filter, $app_define_filter);
	 ...
	 */
	static abstract function group_get_group_member_info($group_id, $limit, $offset);
	 ...
	 * @return array 通过解析REST接口json返回包得到的关联数组，包含成功与否、错误提示等字段
	 */
	static abstract function group_modify_group_base_info($group_id, $group_name);
	 ...
	 * @return array 通过解析REST接口json返回包得到的关联数组，包含成功与否、错误提示等字段
	 */
	static abstract function group_modify_group_base_info2($group_id, $group_name, $info_set, $app_define_list);
	 ...
	 * @return array 通过解析REST接口json返回包得到的关联数组，包含成功与否、错误提示等字段
	 */
	static abstract function group_add_group_member($group_id, $member_id, $silence);
	 ...
	 * @return array 通过解析REST接口json返回包得到的关联数组，包含成功与否、错误提示等字段
	 */
	static abstract function group_delete_group_member($group_id, $member_id, $silence);
	 ...
	 * @return array 通过解析REST接口json返回包得到的关联数组，包含成功与否、错误提示等字段
	 */
	static abstract function group_modify_group_member_info($group_id, $account_id, $role);
	 ...
	 * @return array 通过解析REST接口json返回包得到的关联数组，包含成功与否、错误提示等字段
	 */
	static abstract function group_modify_group_member_info2($group_id, $account_id, $role, $msg_flag, $shutup_time);
	 ...
	 * @return array 通过解析REST接口json返回包得到的关联数组，包含成功与否、错误提示等字段
	 */
	static abstract function group_destroy_group($group_id);
	 ...
	 */
	static abstract function group_get_joined_group_list($account_id);
	 ...
	 * @return array 通过解析REST接口json返回包得到的关联数组，包含该用户加入的群的信息(如果成功), 成功与否、错误提示等字段
	 */
	static abstract function group_get_joined_group_list2($account_id, $group_type, $base_info_filter, $self_info_filter);
	 ...
	 */
	static abstract function group_get_role_in_group($group_id, $member_id);
	 ...
	 * @return array 通过解析REST接口json返回包得到的关联数组，包含该用户在某个群的身份(如果成功), 成功与否、错误提示等字段
	 */
	static abstract function group_forbid_send_msg($group_id, $member_id, $second);
	 ...
	 * @return array 通过解析REST接口json返回包得到的关联数组，包含成功与否、错误提示等字段
	 */
	static abstract function group_send_group_msg($account_id, $group_id, $text_content);
	 ...
	 * @return array 通过解析REST接口json返回包得到的关联数组，包含成功与否、错误提示等字段
	 */
	static abstract function group_send_group_msg_pic($account_id, $group_id, $pic_path);
	 ...
	 * @return array 通过解析REST接口json返回包得到的关联数组，包含成功与否、错误提示等字段
	 */
	static abstract function group_send_group_msg2($account_id, $group_id, $msg_content);
	 ...
	 * @return array 通过解析REST接口json返回包得到的关联数组，包含成功与否、错误提示等字段
	 */
	static abstract function group_send_group_system_notification($group_id, $content, $receiver_id);
	 ...
	 * @return array 通过解析REST接口json返回包得到的关联数组，包含成功与否、错误提示等字段
	 */
	static abstract function group_send_group_system_notification2($group_id, $content, $receiver_list);
	 ...
	 * @return array 通过解析REST接口json返回包得到的关联数组，包含成功与否、错误提示等字段
	 */
	static abstract function group_import_group_member($group_id, $member_id, $role);
	 ...
	 * @return array 通过解析REST接口json返回包得到的关联数组，包含成功与否、错误提示等字段
	 */
	static abstract function group_import_group_member2($group_id, $member_list);
	 ...
	 * @return array 通过解析REST接口json返回包得到的关联数组，包含成功与否、错误提示等字段
	 */
    static abstract function group_import_group_msg($group_id, $from_account, $text);
     ...
     * @return array 通过解析REST接口json返回包得到的关联数组，包含成功与否、错误提示等字段
     */
    static abstract function group_import_group_msg2($group_id, $msg_list);
	 ...
	 * @return array 通过解析REST接口json返回包得到的关联数组，包含成功与否、错误提示等字段
	 */
	static abstract function group_set_unread_msg_num($group_id, $member_account, $unread_msg_num);
	 ...
	 * @return array 通过解析REST接口json返回包得到的关联数组, 其中包含成功与否、及错误提示(如果有错误)等字段
	 */
	static abstract function comm_rest($server_name, $command, $req_body);
}
?>
