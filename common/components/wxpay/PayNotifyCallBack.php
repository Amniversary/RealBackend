...
use common\components\wxpay\lib\WxPayNotify;
...
class PayNotifyCallBack extends WxPayNotify
{
	...
	public function NotifyProcess($data, &$msg)
	{
        ...
        {
            \Yii::getLogger()->log('wxweb支付通知时body参数为空',Logger::LEVEL_ERROR);
        }
        else
        {
            $body = $params['body'];
            ...
            for($i = 0; $i < $len; $i++ )
            {
                ...
                {
                    \Yii::getLogger()->log('wxweb支付通知时body参数解析异常，出现多个等号，原来参数：'.$parItems[$i],Logger::LEVEL_ERROR);
                }
            }
            unset($params['body']);
            if(!isset($params['pay_target']) || empty($params['pay_target']))
            {
                \Yii::getLogger()->log('wxweb支付通知时body参数解析异常，pay_target参数为空',Logger::LEVEL_ERROR);
            }
            else
            {
                ...
                {
                    \Yii::getLogger()->log('wxweb支付通知时结果处理异常：'.$error,Logger::LEVEL_ERROR);
                    $msg = $error;
                    return false;
                }
            }
