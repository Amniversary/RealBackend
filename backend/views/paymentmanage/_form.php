...
<div class="user-form">
    ...
    <div class="form-group field-user-pic1 <?=($model->getFirstError('pic_url') === null?'has-success':'has-error')?>">
        <label class="control-label" for="user-pic1">支付方式图片</label> <a class="pic-del" href="javascript:delpic('pic')">删除</a>
        <input type="hidden" name="Payment[icon]" id="user_pic" value="<?=$model->icon?>"/>
        <input class="backend-pic-input" type="file" class="user-pic-file" id="pic-file-pic"  targetctr="pic">
        <a target="_blank" href="#" id="a-pic" style="<?=empty($model->icon)?'display: none;':''?>">
            <img class="user-pic" src="<?=$model->icon?>" alt="图像">
        </a>
    ...
    <?= $form->field($model, 'status')->dropDownList(['0'=>'禁用','1'=>'正常','2'=>'审核中'],
        ['style'=>'width:120px']) ?>
    ...
    <div class="form-group">
        ...
        &nbsp;&nbsp;&nbsp;&nbsp;
        <?= Html::a('取消',\Yii::$app->urlManager->createUrl(['paymentmanage/index']), ['class' => $model->isNewRecord ? 'btn btn-primary' : 'btn btn-primary']) ?>
    </div>
