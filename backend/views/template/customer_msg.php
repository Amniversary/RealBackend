...
$gridColumns = [
    ...
    [
        ...
        'urlCreator' => function ($action, $model, $key, $index) {
            ...
            switch ($action) {
                case 'update':
                    $url = '/template/update_msg?record_id=' . strval($model->record_id);
                    break;
                case 'delete':
                    $url = '/template/delete_msg?record_id=' . strval($model->record_id);
                    break;
            }
            return $url;
        },
        ...
        'buttons' => [
            'update' => function ($url, $model) {
                return Html::a('修改', $url, ['class' => 'back-a updp-t', 'style' => 'margin-right:3%']);
            },
            'delete' => function ($url, $model) {
                return Html::a('删除', $url, ['class' => 'delete back-a', 'data-toggle' => false, 'data-method'=>'post', 'data-pjax'=>'1']);
            },
        ],
    ],
...
echo GridView::widget([
    ...
    'toolbar' => [
        [
            'content' => Html::button('返回', ['class' => 'btn btn-success', 'onclick' => 'location="' . Yii::$app->urlManager->createUrl('template/batch_customer') . '"; return false;']) .
                Html::button('添加回复消息', ['id' => 'cry-msg', 'type' => 'button', 'title' => '添加回复', 'class' => 'btn btn-success', 'onclick' => 'location="' . \Yii::$app->urlManager->createUrl(['template/create_params_msg', 'id' => $id]) . '"; return false;']),
        ],
...
$(document).ready(function(){
    $vue = $(".updp-t").attr("href") + "&id='.$id.'";
    $(".updp-t").attr("href", $vue);
});
$("#customer-pjax").on("click", ".delete", function() {
...
$url = $(this).attr("href");
    $.ajax({
        ...
        success: function(data) {
            data = $.parseJSON(data);
            if (data.code == "0") {
                $("#customer").yiiGridView("applyFilter");
            } else {
                alert("删除失败：" + data.msg);
            }
        },
