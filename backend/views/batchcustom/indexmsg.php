...
$gridColumns = [
    ...
    [
        ...
        'urlCreator'=> function($action, $model , $key , $index){
            ...
            switch ($action){
                case 'update':
                    $url = '/batchcustom/updatemsg?record_id='.strval($model->record_id).'&menu_id='.strval($model->menu_id);
                    break;
                case 'delete':
                    $url = '/batchcustom/deletemsg?record_id='.strval($model->record_id);
                    break;
            }
            return $url;
        },
        ...
        'buttons'=>[
            'update'=>function($url, $model){
                return Html::a('修改', $url,['class'=>'back-a updt-bt','style'=>'margin-right:10px']);
            },
...
echo GridView::widget([
    ...
    'toolbar'=> [
        [
            'content'=>Html::button('返回',['type'=>'button','class'=>'btn btn-success','onclick'=>'location="'.\Yii::$app->urlManager->createUrl(['batchcustom/index_menu','menu_id'=>$menu_id,'id'=>$id]).'";return false;']).
                Html::button('添加回复消息',['id'=>'cry-msg','type'=>'button','title'=>'添加回复', 'class'=>'btn btn-success','onclick'=>'location="'.\Yii::$app->urlManager->createUrl(['batchcustom/createmsg','menu_id'=>$menu_id,'id'=>$id]).'";return false;']),
        ],
