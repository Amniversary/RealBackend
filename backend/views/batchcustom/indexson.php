...
echo GridView::widget([
    ...
    'toolbar'=> [
        [
            'content'=> Html::button('返回',['type'=>'button','class'=>'btn btn-success','onclick'=>'location="'.\Yii::$app->urlManager->createUrl(['batchcustom/index_menu','id'=>$id]).'";return false;']).'&nbsp;&nbsp;&nbsp;&nbsp;'.
...
$(document).on("click","#create-menu",function(){
    $url = "http://"+ window.location.host + "/batchcustom/check_menu_son?menu_id='.$menu_id.'";
    $.ajax({
        type:"POST",
        url:$url,
        data: "",
        success: function(data){
            data = $.parseJSON(data);
            if(data.code == 0){
                location = "'.\Yii::$app->urlManager->createUrl(['batchcustom/createson','menu_id'=>$menu_id,'id'=>$id]).'";
                return false;
            }else{
                alert(data.msg);
            }
