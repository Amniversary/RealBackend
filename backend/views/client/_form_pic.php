...
<div class="user-form">
    ...
    <div class="form-group field-user-pic1 <?=($model->getFirstError('pic') === null?'has-success':'has-error')?>">
        <label class="control-label" for="user-pic1">成员封面图片</label> <a class="pic-del" href="javascript:delpic('pic')">删除</a>
        <input type="hidden" name="Client[pic]" id="user_pic" value="<?=$model->pic?>"/>
    ...
    <div class="form-group">
        ...
        &nbsp;&nbsp;&nbsp;&nbsp;
        <?= Html::a('取消',\Yii::$app->urlManager->createUrl(array_merge(['client/client_cover_index'],$params)), ['class' => $model->isNewRecord ? 'btn btn-success' : 'btn btn-primary']) ?>
    </div>
