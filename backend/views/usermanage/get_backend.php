...
?>
    ...
    <div class="check-title">
        <p><?= '设置账户【' . strval($user->username) . '】的权限' ?></p>
    </div>
    <div class="form-group check-button-list">
        <?php
        if ($haveSetPower) {
            echo Html::button('&nbsp;', ['class' => 'btn_check_all']) . '&nbsp;&nbsp;'
                . Html::button('设置权限', ['class' => 'btn btn-success check-refuse', 'id' => 'btn_refuse']) . '&nbsp;&nbsp;';
        }
...
$(".check-refuse" ).on("click",function(){
        ...
        else
        {
            ...
            {
                $("#has_submit").val("1");
                $url = "/usermanage/set_backend?user_id=' . $user->backend_user_id . '";
...
$(function(){
$("#contact-backend").on("hide.bs.modal",function(e){$(this).removeData();});
});
';
if (!$haveSetPower) {
