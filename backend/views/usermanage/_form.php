...
<?php
    ...
    <div class="form-group field-user-pic1 has-success">
        <label class="control-label" for="user-pic1">头像</label> <a class="pic-del" href="javascript:delpic('pic')">删除</a>
        <input type="hidden" name="User[pic]" id="user_pic" value="<?=$model->pic?>"/>
        <input class="backend-pic-input" type="file" class="user-pic-file" id="pic-file-pic"  targetctr="pic">
        <a target="_blank" href="<?=$model->pic?>" id="a-pic" style="<?=empty($model->pic)?'display: none;':''?>">
    ...
    <div class="form-group">
        ...
        &nbsp;&nbsp;&nbsp;&nbsp;
        <?= Html::a('取消',\Yii::$app->urlManager->createUrl(['usermanage/index']), ['class' => $model->isNewRecord ? 'btn btn-success' : 'btn btn-primary']) ?>
    </div>
