...
<div class="user-form">
    ...
    <?= $form->field($model, 'gold_goods_name')->textInput() ?>
    ...
    <div class="form-group field-user-pic1 <?=($model->getFirstError('pic_url') === null?'has-success':'has-error')?>">
        ...
        <input type="hidden" name="GoldsGoods[gold_goods_pic]" id="user_pic" value="<?=$model->gold_goods_pic?>"/>
        <input class="backend-pic-input" type="file" class="user-pic-file" id="pic-file-pic"  targetctr="pic">
            <a target="_blank" href="#" id="a-pic" style="<?=empty($model->gold_goods_pic)?'display: none;':''?>">
            <img class="user-pic" src="<?=$model->gold_goods_pic?>" alt="图像">
        </a>
        <div class="help-block"><?=$model->getFirstError('pic')?></div>
    </div>

    <?= $form->field($model, 'gold_goods_price')->textInput() ?>

    <?= $form->field($model, 'gold_num')->textInput() ?>

    <?= $form->field($model, 'extra_integral_num')->textInput() ?>

    <?= $form->field($model, 'order_no')->textInput() ?>
    <div class="form-group">
        <?= Html::submitButton($model->isNewRecord ? '新增' : '修改', ['class' => $model->isNewRecord ? 'btn btn-success' : 'btn btn-primary']) ?>
        &nbsp;&nbsp;&nbsp;&nbsp;
        <?= Html::a('取消',\Yii::$app->urlManager->createUrl(['goldsgoods/index']), ['class' => $model->isNewRecord ? 'btn btn-success' : 'btn btn-primary']) ?>
    </div>
