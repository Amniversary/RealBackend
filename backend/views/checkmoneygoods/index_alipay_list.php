...
echo Tabs::widget([
    ...
    'items' => [
        [
            'label' => '未打款',
            'url' => \Yii::$app->urlManager->createAbsoluteUrl(['checkmoneygoods/cash_alipay_unpaid']),
            'active' => ($data_type === 'unpaid'? true: false),
            'options' => ['id' => 'my_comment_undo'],
        ],
        [
            'label' => '已打款',
            'url' =>\Yii::$app->urlManager->createAbsoluteUrl(['checkmoneygoods/cash_alipay_paid ']),
...
//    ],
    ...
    [
        ...
        {
            $data_type = $model['status']==2?'unpaid':'paid';
            $url = '';
            switch($action)
            {
                case 'update':
...
$(document).on("click",".select_pass",function()
{
    length = $(".select_check:checked").length;
        ...
        var ss = 0;
        elems.each(function() {
            ss += parseFloat($(this).siblings().eq(9).text());
        });
        console.log(elems.siblings().eq(9).text());
    var flag = false;
    var dig3 = artDialog
    ({
            title: "提示",
