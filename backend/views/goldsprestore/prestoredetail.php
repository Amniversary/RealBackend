...
<div class="relate-contain">
    <ul class="user-item"><li class="user-item-detail cash_type" data-type="">金币商品名称：<?= $model->gold_goods_name ?> </li><li class="user-item-detail"></ul>
    <ul class="user-item"><li class="user-item-detail">金币商品单价：<?= $model->gold_goods_price ?>元</li><li class="user-item-detail"></ul>
    <ul class="user-item"><li class="user-item-detail">金币商品数量：<?= $model->gold_goods_num ?></li><li class="user-item-detail"></ul>
...
$js = '
    $(".check-pass").on("click",function(){
        if(confirm("确定检验充值结果吗？"))
        {
            ...
            $url = "/goldsprestore/check_prestore_recode";
            dataStr = "prestore_id='.$model->prestore_id.'&pay_type='.$model->pay_type.'";
