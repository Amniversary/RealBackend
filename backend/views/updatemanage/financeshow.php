...
?>
    ...
    <div class="check-title">
        <p><?='将借款单号【'.strval($bill_info->borrow_fund_id).'】的第【'.strval($bill_info->cur_stage).'】期设置成坏账'?></p>
    </div>
    <div class="form-group check-button-list">
        <?= Html::button('设置坏账' , ['class' =>'btn btn-success check-refuse','id'=>'btn_refuse']).'&nbsp;&nbsp;'.Html::button('返回' , ['data-dismiss'=>'modal','aria-hidden'=>'true','class' =>'btn btn-success'])
...
$(".check-refuse").on("click",function(){
        ...
        if(confirm("确定设置成坏账吗？"))
        {
            ...
            $("#has_submit").val("1");
            $url = "/mybill/remarkbad?my_bill_id='.$bill_info->bill_id.'";
