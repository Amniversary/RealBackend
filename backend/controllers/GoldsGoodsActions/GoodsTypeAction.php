...
/**
 * 修改金币商品
 * Class GoodsTypeAction
 * @package backend\controllers\GoodsTypeAction
 */
class GoodsTypeAction extends Action
{
    public function run($gold_goods_id){
        ...
        if(!isset($dataItem['gold_goods_type']))
        {
            $rst['message'] = '商品类型值为空';
            echo json_encode($rst);
            exit;
        }
        $gold_goods_type = $dataItem['gold_goods_type'];
        $goods->gold_goods_type = $gold_goods_type;
        if(!$goods->save()){
            $rst['message'] = '更新错误';
            echo json_encode($rst);
            exit;
