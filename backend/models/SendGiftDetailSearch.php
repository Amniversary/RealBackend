...
class SendGiftDetailSearch extends GiftDetailForm{
	...
	public function search($params){
        ...
        $query = (new Query())
            ->select(['r.multiple','r.total_gift_value','r.receive_rate','r.gift_type','r.gift_value','r.gift_name','r.living_master_id', 'c.client_no', 'c.nick_name', 'cb.before_balance', 'cb.after_balance', 'r.create_time', 'cb.device_type'])
            ->from('mb_reward r')
            ->innerJoin('mb_client c','r.living_master_id = c.client_id')
            ->innerJoin('mb_client_balance_log cb','r.reward_id = cb.relate_id and cb.operate_type = 6')
            ->orderBy(['r.create_time' => SORT_DESC  ,'r.reward_id' => SORT_DESC])
            ->where('r.reward_user_id=:id', [':id' => $user_id]);
