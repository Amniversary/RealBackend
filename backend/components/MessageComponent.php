...
class MessageComponent
{
        ...
        if (AuthorizerUtil::isVerify($this->auth->verify_type_info)) {
            ...
            foreach ($keyword as $item) {
                ...
                if ($touch) {
                    ...
                    if($item['global'] == '4') {
                        $msgData = $this->setLaterFlag($error);
                        if(!$msgData) {
                            \Yii::error($error);
                        }
                        break;
    ...
    public function VerifyKeyWord()
    {
        ...
        foreach ($keyword as $item) {
            ...
            if ($touch) {
                ...
                if($item['global'] == '4') {
                    $params = ['key_word'=> 'later_sign', 'key'=> $item['key_id'], 'data'=> $this->data];
                    if (!JobUtil::AddCustomJob('imgBeanstalk', 'gen_later_img', $params, $error)) {
                        \Yii::error($error);
                        return null;
                    }
                }
     ...
     * @return array
     */
    public function getLaterParams()
    {
        $query = (new Query())
            ->select(['pic_url'])
            ->from('wc_later_keyword lk')
            ->innerJoin('wc_later_image li', 'lk.later_id=li.later_id')
            ->where('lk.key_id = :kd', [':kd'=>$this->key])->all();
        return $query;
    }
    /**
     * 更新用户签到记录
     ...
     * @return bool
     */
    public function setLaterFlag(&$error)
    {
        $accessToken = $this->auth->authorizer_access_token;
        $openId = $this->data['FromUserName'];
        $later_params = $this->getLaterParams();
        if (empty($later_params) || !isset($later_params)) {
            return false;
        }
        $count = count($later_params);
        $pic = $later_params[rand(0, ($count - 1))]['pic_url'];
        $res = UsualFunForNetWorkHelper::HttpGetImg($pic, $type, $error);
        if (!$res) {
            $error = '获取早晚安图片Url 失败';
            \Yii::error($error . '  pic:' . $pic);
            return false;
        }
        $file  = \Yii::$app->basePath . '/runtime/later/pic_' . $openId . '.jpg';
        if (!file_put_contents($file, $res)) {
            $error = '保存早晚安图片到本地失败';
            return false;
        }
        if (!ImageUtil::imageLater($file, $filename, $error)) {
            \Yii::error($error);
            return false;
        }
        if (!file_exists($filename)) {
            $error = '用户早晚安图片生成失败  Pic:' . $filename;
            \Yii::error($error . '  Pic :' . $filename);
            return false;
        }
        $wechat = new WeChatUtil();
        if (!$wechat->Upload($filename, $accessToken, $rst, $error)) { //TODO: 背景图上传微信素材
        ...
        //fwrite(STDOUT, Console::ansiFormat("执行时间 Time : " . (microtime(true) - $start) . "\n", [Console::FG_BLUE]));
//        $this->sendMessageCustom($msgData, $openId);
        return $msgData;
    }
