...
class GenSignUpImgAction extends Action
{
    public function run($job)
    {
        ...
        try {
            ...
            $msgObj = new ImageClass($data);
            if(!$msgObj->getSignUpImg($error))
            {
                ...
                \Yii::error('任务处理失败，jobid：'.$jobId.'--- :'.$error .' openId :'.$data['FromUserName'] . ' AppId : '. $data['appid']);
