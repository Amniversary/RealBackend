...
use frontend\business\SaveRecordByTransactions\SaveByTransaction\TemplateSaveByTrans;
...
class TemplateController extends Controller
{
    public function actionSendtemplatemsg()
    {
        ...
        if(!empty($OneGroupSql)) {
            foreach($OneGroupSql as $one) {
                ...
                if($res['errcode'] != 0) {
                    ...
                    echo "Code : ". $res['errcode'] . '  msg :'. $res['errmsg'] ."\n";
                    continue;
                }
                //TODO : 处理用户标记
                $data = ['client_id'=>$one['client_id'], 'app_id'=> $one['app_id'],'flag'=>1];
                $transAction[] = new TemplateSaveByTrans($data);
                $OneCount ++;
            }
        ...
        if(!empty($TowGroupSql)) {
            foreach($TowGroupSql as $tow) {
                ...
                $URL = 'http://novel.duobb.cn/novel/vipremind?app='.$num;
                $msg = $template->BuildTemplate($tow['open_id'],$template_id,$templateMsg,$URL);
                $res = $template->SendTemplateMessage($msg);
                if($res['errcode'] != 0) {
                ...
                $transActions[] = new TemplateSaveByTrans($data);
                $TowCount ++;
            }
    ...
    private function GetOneGroupUserList($str) {
        $sql = 'select client_id,nick_name,open_id,c.app_id, ifnull(temp_num,0) as temp_num
                ...
                where  is_vip = 0 and c.app_id in ('.$str.') and (create_time BETWEEN :star and :end) and subscribe = 1';
