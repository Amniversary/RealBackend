 ...
 *
 */
class ClickLikeAction extends Action
{
    public function run($job)
    {
        ...
        fwrite(STDOUT, Console::ansiFormat("---ClickLike-- in job id:[$jobId]---[$new_job_id]----"."\n", [Console::FG_GREEN]));
        try
        {
            $jobRecord = JobUtil::GetJobById($new_job_id);
            if(!isset($jobRecord))
            {
                if(!JobUtil::AddJobToDb($jobId,$new_job_id,'click_like',$sentData,$error,$jobRecord))
                {
                    fwrite(STDOUT, Console::ansiFormat("---clickLike--  error no jobrecord"."\n", [Console::FG_GREEN]));
...
//                $jobRecord = JobUtil::GetJobById($new_job_id);
            ...
            $transActions = new UpdateClickLikeSaveForReward($sentData);    //更新热门直播信息
...
//            $transActions[] = new LivingHotModifyByTrans($LivingHotUtil);    //更新热门直播信息
            if(!$transActions->SaveRecordForTransaction($error,$out))
            {
                ...
                fwrite(STDOUT, Console::ansiFormat("---ClickLike-11- error:$error"."\n", [Console::FG_GREEN]));
            ...
            if($everthingIsAllRight == true){
                fwrite(STDOUT, Console::ansiFormat("---ClickLike--  Everything is allright"."\n", [Console::FG_GREEN]));
