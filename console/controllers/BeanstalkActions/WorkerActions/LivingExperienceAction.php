...
/**
 * lxy 退出直播经验处理
 * Class LivingExperienceAction
 * @package console\controllers\BeanstalkActions\WorkerActions
 */
class LivingExperienceAction extends Action
{
    public function run($job)
    {
        ...
        fwrite(STDOUT, Console::ansiFormat("---living_experience-- in job id:[$jobId]---[$new_job_id]"."\n", [Console::FG_GREEN]));
        try
        {
            $jobRecord = JobUtil::GetJobById($new_job_id);
            if(!isset($jobRecord))
            {
                if(!JobUtil::AddJobToDb($jobId,$new_job_id,'living_experience',$sentData,$error,$jobRecord))
                {
                    fwrite(STDOUT, Console::ansiFormat("---living_experience--  error no jobrecord"."\n", [Console::FG_GREEN]));
                    return BeanstalkController::DELETE;
                }
//                $jobRecord = JobUtil::GetJobById($new_job_id);
            }
            // something useful here
           if(!ClientUtil::BeanTalkdExperience($new_job_id,$sentData,$error))
           {
               $error1 = $error;
               ...
               fwrite(STDOUT, Console::ansiFormat("---living_experience--  error:$error"."\n", [Console::FG_GREEN]));
            ...
            if($everthingIsAllRight == true){
                fwrite(STDOUT, Console::ansiFormat("---living_experience--  Everything is allright"."\n", [Console::FG_GREEN]));
