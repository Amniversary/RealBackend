 ...
 */
class HeartBeanAction extends Action
{
    public function run($job)
    {
        ...
        fwrite(STDOUT, Console::ansiFormat("---living_heart-- in job id:[$jobId]---[$new_job_id]"."\n", [Console::FG_GREEN]));
        try
        {
            $jobRecord = JobUtil::GetJobById($new_job_id);
            if(!isset($jobRecord))
            {
                if(!JobUtil::AddJobToDb($jobId,$new_job_id,'living_heart',$sentData,$error,$jobRecord))
                {
                    fwrite(STDOUT, Console::ansiFormat("---living_heart--  error no jobrecord"."\n", [Console::FG_GREEN]));
...
//                $jobRecord = JobUtil::GetJobById($new_job_id);
            ...
            if(!HeartBeatUtil::DealHeartBeat($sentData,$error))
            {
               $error1  = $error;
               ...
               fwrite(STDOUT, Console::ansiFormat("---".date('Y-m-d H:i:s')."living_heart--  error:$error"."\n", [Console::FG_GREEN]));
               $jobRecord->remark1 = $error1;
               $jobRecord->status = 2;
               \Yii::getLogger()->log('任务处理失败，jobid：'.$jobId.'----'.$new_job_id.' :'.$error1,Logger::LEVEL_ERROR);
               if(!$jobRecord->save())
               {
            ...
            if($everthingIsAllRight == true){
                fwrite(STDOUT, Console::ansiFormat("--".date('Y-m-d H:i:s')."--living_heart--  Everything is allright, living_id:".$sentData['living_id'].' user_id:'.$sentData['user_id']."\n", [Console::FG_GREEN]));
            ...
            $jobRecord->remark1 = $e->getMessage();
            $jobRecord->status = 2;
            \Yii::getLogger()->log('任务处理失败，jobid：'.$jobId.' :'. $e->getMessage(),Logger::LEVEL_ERROR);
            if(!$jobRecord->save())
            {
                \Yii::getLogger()->log('保存任务状态失败4 ：'.var_export($jobRecord->getErrors(),true),Logger::LEVEL_ERROR);
            }
