...
class ZhiBoSendGift implements IApiExcute
{
    ...
    public function excute_action($dataProtocal, &$rstData,&$error, $extendData= array())
    {
        ...
        if(!isset($dataProtocal['data']['op_unique_no']) || empty($dataProtocal['data']['op_unique_no']))
        {
            $error = '唯一操作号不能为空';
            return false;
        }

        $gift_object = GiftUtil::GetGiftById($dataProtocal['data']['gift_id']);
        ...
        if($dataProtocal['data']['money_type'] == 1)
        {
            ...
            {
                $error = '鲜花余额不足';
                \Yii::getLogger()->log('$banlances_object==:'.var_export($banlances_object,true),Logger::LEVEL_ERROR);
                return false;
            }
        }
        //虚拟
