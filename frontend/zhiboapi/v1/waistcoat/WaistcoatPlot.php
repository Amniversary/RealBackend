...
class WaistcoatPlot
{
    ...
    public function DoAction(){
        ...
        $error  = '';
        $configFile = \Yii::$app->getBasePath().'/zhiboapi/v1/waistcoat/WaistcoatConfig.php';
        ...
        if(!isset($funData[$className]))
        {
            ...
            \Yii::getLogger()->log('v1找不到对应的处理类:'.$className,Logger::LEVEL_ERROR);
            echo json_encode($rst);
            exit;
        }

        $actionClass = 'frontend\zhiboapi\v1\waistcoat\\'.$funData[$className];
