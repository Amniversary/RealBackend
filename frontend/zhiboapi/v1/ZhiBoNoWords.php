...
//\Yii::getLogger()->log(var_export($dataProtocal, true),Logger::LEVEL_ERROR);
        ...
        $is_police = $loginInfo['client_type'] == '2';
        ...
        try {
            $chatroomHelper->setNospeakingInChatroom($shutup_user_id, $user_id, $living_id, 300, $is_police);
        } catch (\Exception $e) {
            $error = $e->getMessage();
            return false;
        }
        ...
        $rst = $chatroomHelper->sendChatroomSuperMsg($living_id, [
            'user_id' => $client_info->client_id,
            'nick_name' => $client_info->nick_name,
        ], $chatroomHelper::MSG_NOSPEAKING_TAG, [], $rstData['data']['message']);
        if ($rst !== true) {
            $error = $rst;
            return false;
        }
