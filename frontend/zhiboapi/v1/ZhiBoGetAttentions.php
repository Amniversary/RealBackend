 ...
 */
class ZhiBoGetAttentions implements IApiExcute
{
    /**
     * 检查参数合法性
     * @param string $error
    ...
    public function excute_action($dataProtocal, &$rstData,&$error, $extendData= array())
    {
        ...
        $friendsList = ChatFriendsUtil::GetAttentions($user_id,$page_no,$page_size,$loginInfo['user_id']);
        //$out = json_encode($friendsList);
        $rstData['has_data'] = '1';
        $rstData['data_type']="jsonarray";
        $rstData['data']=$friendsList;
        //\Yii::getLogger()->log('关注列表:'.var_export($friendsList, true),Logger::LEVEL_ERROR);
        //根据经度、纬度获取地理信息
        return true;
    }
} 