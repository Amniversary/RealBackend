...
//        \Yii::getLogger()->log('进入直播间living_enter_room_living_id==:'.$living_id.'   user_id==:'.$user_id,Logger::LEVEL_ERROR);
        ...
        if(!ChatGroupUtil::QiNiuEnterRoom($living_id,$user_id,$deviceType,$owner,$error,0,$is_police,'test-ry'))
        {
            return false;
        }
        ...
        $livingInfo = LivingUtil::GetLivingImInfo($error['living_master_id']);
        ...
        if (!empty($livingInfo)) {
            $rstData['data']['ticket_count_sum'] = $livingInfo['ticket_count_sum'];
            $rstData['data']['person_count'] = $livingInfo['person_count'];
        }
        //根据经度、纬度获取地理信息
