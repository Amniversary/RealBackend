...
class ZhiBoCreateLivingApprove implements IApiExcute
{
    ...
    public function excute_action($dataProtocal, &$rstData,&$error, $extendData= array())
    {
        ...
        $approve_phone = Approve::findOne(['phone_num' => $dataProtocal['data']['phone_number'],'status'=>1]);

        if($user_info['is_centification'] > 1)
        {
            $error = '您已经认证通过了';
            return false;
        }
        if(!empty($approve_phone))
        {
            $error = '该手机号码已注册';
            return false;
        }
...
//        }
        ...
        $InsertSql = 'insert ignore into mb_approve(client_id,client_no,status) VALUES (:cid,:cn,0)';
        \Yii::$app->db->createCommand($InsertSql,[
            ':cid' => $user_info['client_id'],
            ':cn' => $user_info['client_no'],
        ])->execute();
        $sqls = 'update mb_approve set create_time=:ctime,phone_num=:pn,status=1 WHERE client_id=:cid';
