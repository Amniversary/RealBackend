...
class ZhiBoFansApplyApprove implements IApiExcute
{
    ...
    public function excute_action($dataProtocal, &$rstData, &$error, $extendData= array())
    {
        ...
        //获得用户的id
        $admin_id = $sysLoginInfo['user_id'];
        $group_id = $dataProtocal['data']['group_id'];
        $member = FansGroupMember::findOne(['user_id'=>$admin_id, 'group_id'=>$group_id]);
        ...
        $groupHelper = new \frontend\business\RongCloud\GroupUtil();

        try {
            $groupHelper->apply($user_id, $admin_id, $group_id, $apply_status);
        } catch (\Exception $e) {
            $error = $e->getMessage();
            return false;
        }
