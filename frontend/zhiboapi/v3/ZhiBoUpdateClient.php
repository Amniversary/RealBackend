...
class ZhiBoUpdateClient implements IApiExcute{
    ...
    public function excute_action($dataProtocal, &$rstData,&$error, $extendData= array())
    {
        ...
        {
            $inviter_info = ClientUtil::GetClientNo(strval($dataProtocal['data']['inviter_id']));
        ...
        $rst = $RongCloud->getToken($LoginInfo['user_id'], $data['nick_name'], $data['pic']);
        if(!$rst)
        {
            $error = $RongCloud->getErrorMessage();
            return false;
        }
        $data['token'] = $rst['token'];
