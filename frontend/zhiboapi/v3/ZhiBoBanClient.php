...
class ZhiBoBanClient implements IApiExcute
{
    ...
    public function excute_action($dataProtocal, &$rstData,&$error, $extendData= array())
    {
        ...
        if($living_info['status'] == 2)
        {
            ...
            $finishInfo = null;
            if(!LivingUtil::SetBanClientFinishLivingToStopLiving($living_info['living_id'],$finishInfo,$living_info['living_master_id'],$living_info['other_id'],$outInfo,$error))
            {
                return false;
            }
        }
        ...
        //IM禁止发言
        if(!TimRestApi::setnospeaking($dataProtocal['data']['client_id'])) {
            return false;
        }
