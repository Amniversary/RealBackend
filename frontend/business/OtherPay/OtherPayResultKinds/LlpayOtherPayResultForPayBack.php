...
class LlpayOtherPayResultForPayBack implements IOtherPayResult
{
    public function DoOtherPayResult($params, &$error)
    {
        ...
        {
            //支付失败不做处理
            $error = '还款连连支付失败，单号：'.$params['out_trade_no'];
            return false;
        }
        //检测参数
        ...
        for($i =0; $i <$len; $i ++)
        {
            if(!isset($fields[$i]))
            {
                $error = $fieldLabels[$i].'不能为空，连连支付还款参数丢失';
                return false;
            }
        }
