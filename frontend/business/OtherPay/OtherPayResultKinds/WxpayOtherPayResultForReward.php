...
class WxpayOtherPayResultForReward implements IOtherPayResult
{
    public function DoOtherPayResult($params, &$error)
    {
        ...
        {
            //支付失败不做处理
            $error = '打赏微信支付失败，单号：'.$params['out_trade_no'];
        ...
        if(isset($user))
        {
            ...
            if(PersonalUserUtil::IsFirstTimeReward($user,true))
            {
                ...
                {
                    \Yii::getLogger()->log('wxpay用户第一次打赏领取红包异常'.$error.' user_id:'.strval($user_id), Logger::LEVEL_ERROR);
                }
            }
        }
        else
