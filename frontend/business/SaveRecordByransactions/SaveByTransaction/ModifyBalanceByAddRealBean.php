...
class ModifyBalanceByAddRealBean implements ISaveForTransaction
{
    ...
    public function SaveRecordForTransaction(&$error,&$outInfo)//($params,&$balance, &$error)
    {
        ...
        if(!isset($params['bean_num']) || empty($params['bean_num']) || doubleval($params['bean_num'])<= 0)
        {
            //从外面穿进来
            if(isset($outInfo) &&
                isset($outInfo['op_value']) &&
                doubleval($outInfo['op_value']) > 0)
            {
                $params['bean_num'] = $outInfo['op_value'];
            }
            else
            {
                $error = '鲜花必须大于0';
...
';
        ...
        {
            $error = '增加鲜花失败';
            \Yii::getLogger()->log($error.'user_id:['.$this->balance->user_id.'] sql:'.\Yii::$app->db->createCommand($sql,[
                    ':am'=>$params['bean_num'],
                    ':uid'=>$this->balance->user_id
