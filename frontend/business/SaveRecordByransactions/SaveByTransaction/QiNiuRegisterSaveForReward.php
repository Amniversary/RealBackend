...
class QiNiuRegisterSaveForReward implements ISaveForTransaction
{
    ...
    function SaveRecordForTransaction(&$error,&$outInfo)
    {
        ...
        {
            $error = '用户数据保存失败';
            \Yii::getLogger()->log($error.': '.var_export($this->getClientRecord->getErrors(),true),Logger::LEVEL_ERROR);
            return false;
        }
        ...
        if($this->getClientRecord->register_type != 1)
        {
            ...
            {
                if($this->getClientRecord->is_bind_weixin == 2)
                {
                ...
                if(!empty($user_id) && $user_id != $this->getClientRecord->client_id )
                {
        ...
        //$pwd = Des3Crypt::des_encrypt($srt,$pwd);
        ...
        //新用户注册时，同时开通金币帐户及积分帐户
        GoldsAccountUtil::CreateAnGoldsAccountForNewRegisUser($this->getClientRecord->client_id);
        IntegralAccountUtil::CreateAnIntegralAccountForNewRegisUser($this->getClientRecord->client_id);

        //七牛直播信息 lxy 20160719
        $clientQiniu = new ClientQiniu();
        $clientQiniu->user_id = $this->getClientRecord->client_id;
        $clientQiniu->qiniu_info = $this->extend_params['qiniu_info'];
        if(!$clientQiniu->save())
        {
            $error ='保存七牛直播信息异常';
