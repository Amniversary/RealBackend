...
class UpdateLivingSaveByTrans implements ISaveForTransaction
{
    private  $data=[];
    ...
    function SaveRecordForTransaction(&$error,&$outInfo)
    {
        ...
        $sql = 'update mb_living set `create_time`=if(status=2,create_time,:time),remark3=:rm3,`status`=2,`push_type`=:ptype,`push_url`=:url,`pull_http_url`=:hturl,`pull_rtmp_url`=:rurl,`pull_hls_url`=:hlurl,`is_to_expirence`=:ce
         where living_id=:id';

        if(strtolower(($this->data['data']['push_url'] == 'null')) || empty($this->data['data']['push_url'])){
            $this->data['data']['push_url'] = '';
        }

        if(strtolower(($this->data['data']['pull_http_url'] == 'null')) || empty($this->data['data']['pull_http_url'])){
            $this->data['data']['pull_http_url'] = '';
        }

        if(strtolower(($this->data['data']['pull_rtmp_url'] == 'null')) || empty($this->data['data']['pull_rtmp_url'])){
            $this->data['data']['pull_rtmp_url'] = '';
        }

        if(strtolower(($this->data['data']['pull_hls_url'] == 'null')) || empty($this->data['data']['pull_hls_url']))
        {
            $this->data['data']['pull_hls_url'] = '';
        }


        \Yii::getLogger()->log('更新直播状态为正在直播，living_id:'.$this->data['data']['living_id'],Logger::LEVEL_ERROR);
